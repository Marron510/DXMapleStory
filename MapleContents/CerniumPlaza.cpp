...
ACerniumPlaza::ACerniumPlaza()
{
...

	{
		...
		Plaza->SetRelativeLocation({ 0, -390, BackMap });
	}
	...
	// 깃발
	{
		...
		Flag0->SetRelativeLocation({ -334.0f, 374.0f , BackImage });
	}
	{
		RootComponent = Default;
		...
		Flag1->SetRelativeLocation({ -800.0f, 484.0f , BackImage });
	}

	{
		...
		Flag2->SetRelativeLocation({ -1302.0f, 638.0f , BackImage });
	}
...

	{
		...
		Flag3->SetRelativeLocation({ 364.0f, 374.0f , BackImage });
	}
	{
		RootComponent = Default;
		...
		Flag4->SetRelativeLocation({ 840.0f, 484.0f , BackImage });
	}

	{
		...
		Flag5->SetRelativeLocation({ 1328.0f, 638.0f , BackImage });
	}
...

	{
		...
		Smithy->SetRelativeLocation({ -1100.0f, -130.0f , Building });
	}

	{
		...
		BuildingLeft_0->SetRelativeLocation({ -1370, -150 , Building - 0.1f });
	}
	
	{
		...
		BuildingLeft_1->SetRelativeLocation({ -1850, -333 , Building - 0.2f });
	}
...

	{
		...
		Potion->SetRelativeLocation({ 1160.0f, -130.0f , Building });
	}

	{
		...
		BuildingRight_0->SetRelativeLocation({ 1400, -150 , Building -0.1f });
	}

	{
		...
		BuildingRight_1->SetRelativeLocation({ 1830, -310 , Building -0.2f });
	}
...

	{
		...
		DawnPriestFlag->SetRelativeLocation({ -1700, -182 , Building });
	}

	{
		...
		FirePriestFlag->SetRelativeLocation({ 1700, -182, Building });
	}

	{
		...
		Tree_0->SetRelativeLocation({ -1860, -182 , Building });
	}

	{
		...
		Tree_1->SetRelativeLocation({ 1890, -182 , Building });
	}
...

	{
		...
		FootHold_Left_0->SetRelativeLocation({ 422, 255 , Building - 0.3f });
	}

	{
		...
		FootHold_Left_1->SetRelativeLocation({ 818, 405 , Building -0.3f });
	}

	{
		...
		FootHold_Mid->SetRelativeLocation({ 954, 404 , Building -0.3f });
	}

	{
		...
		FootHold_Right->SetRelativeLocation({ -782, 405, Building -0.3f });
	}
